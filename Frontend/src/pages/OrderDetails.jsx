...
function OrderDetails() {
  ...
  const subtotal = order.items?.reduce((s, i) => s + i.unit_price * i.quantity, 0) ?? order.total_amount;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
        ...
        <div className="surface">
          <div className="surface-header">
            ...
            <span className="badge badge-brand">{order.items?.length ?? 0} items</span>
          </div>
              ...
              <tbody>
                {(order.items || []).length === 0 ? (
                  ...
                  order.items.map((item) => (
                    <tr key={item.id}>
                      <td><span className="cell-mono">Product #{item.product_id}</span></td>
          ...
          {order.items && order.items.length > 0 && (
            <div style={{
              ...
              <div style={{ display: "flex", gap: 60, fontSize: 13, color: "var(--text-secondary)" }}>
                <span>Subtotal</span>
              ...
              <div style={{ display: "flex", gap: 60, fontSize: 16, fontWeight: 700, color: "var(--text-primary)", borderTop: "1px solid var(--border-color)", paddingTop: 8 }}>
                <span>Total</span>
          ...
          <div className="surface">
            ...
            <div className="surface-body" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                { label: "Total Amount", value: `$${order.total_amount?.toFixed(2)}`, bold: true },
                { label: "Status", badge: true, status: "completed" },
              ].map((row) => (
                <div key={row.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
                  ...
                  {row.badge ? (
                    ...
                    <span style={{ fontWeight: row.bold ? 700 : 400, color: row.bold ? "var(--brand-600)" : "var(--text-primary)" }}>
                      {row.value}
                    </span>
                  )}
                </div>
